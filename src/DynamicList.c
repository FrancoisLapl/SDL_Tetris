...
static void resizeList(DynamicList *list, int multiple) 
{
	...
	if (multiple > 0) // if the multiple is 0 or lower we compact the list instead of expendind his size
...
static bool indexOutOfRange(DynamicList *list, int index) 
{
	assert(list != NULL);
...
void DL_insertAt(DynamicList *list, int index, const void *inObject) 
{
	...
	int length = list->objectSize*(list->count - index + 1);
...
//
// Copy the object at specified position in the list at the outObject pointer address
//
